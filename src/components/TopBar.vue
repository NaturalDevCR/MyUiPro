<template>
  <q-page-container>
    ...
    >
      ...
      >
        ...
        <q-tooltip anchor="center right" self="center left" :offset="[10, 10]" class="bg-dark">
          <span class="text-h6"> Firmware version: {{ mixerStore.mixerInfo.firmware }} </span>
        </q-tooltip>
      </div>
      <q-space />
      <div style="border-color: gray; border-radius: 5px" class="row q-card--bordered q-pa-xs">
        <div v-if="!mixerStore.showPlayerControls" class="q-mr-xl">
          <q-btn
            @click="commonStore.modal.layoutSelector = true"
            color="dark"
      ...
      <q-space />
      <q-fab padding="4px" direction="down" color="dark" icon="mdi-cog" active-icon="mdi-close">
        ...
        <q-fab-action
          ...
          @click="midiStore.midiSetupModal = true"
          icon="mdi-midi"
          color="green"
        >
    ...
    <q-page-sticky class="z-max" style="opacity: 0.8" v-else :position="fabPos" :offset="fabOffset">
      <q-btn
        size="xs"
        @click="
          hideBar = false;
          commonStore.barSize = '55px';
        "
        dense
