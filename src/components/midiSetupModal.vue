  ...
  >
    <q-card style="background: #343434" class="text-white">
      <q-bar style="height: 50px">
        <LanguageSwitcher />
        <q-space />
        <div>Soundcraft {{ mixerStore.mixerInfo.model }}</div>
        <q-space />
        ...
        <q-card dark class="col-12 col-md-12 row justify-center">
          ...
          <q-card-section class="col-12 col-md-4">
            <q-list v-if="midiStore.midiDevices.length" dark bordered>
              ...
              <q-separator dark />
              <q-item
                v-for="(device, index) in midiStore.midiDevices"
                :key="device.id + index"
                clickable
                @click="midiStore.selectMidiDevice(device)"
              >
                ...
                <q-item-section side>
                  <q-item-label caption
                    ><q-icon
                      name="mdi-midi-port"
                      size="md"
                      :color="midiStore.selectedDevice.id === device.id ? 'green' : 'red'"
                  /></q-item-label>
                </q-item-section>
              </q-item>
            </q-list>
          ...
          <q-card-section
            v-if="!!midiStore.selectedDevice.id && mixerStore.isConnected"
            class="col-12 row justify-center content-center items-center"
          >
            <q-table
              flat
              bordered
                ...
                <q-tr :props="props">
                  <q-td key="name" :props="props">
                    ...
                    <span class="text-xs text-warning">{{
                      mixerStore.channelNames[`master${props.row.id}`]
                        ? `[ ${props.row.name} ]`
                        : ''
                    }}</span>
                  </q-td>
                  <q-td key="hiz" :props="props">
                    <q-btn
                      round
                      :icon="buttonIcon('hiz', props.row.id)"
                      @click="toggleListening('hiz', props.row.id)"
                      v-if="props.row.modules.includes('hiz')"
                      :class="buttonColor('hiz', props.row.id)"
                      class="full-width rounded-borders cursor-pointer"
                    >
                      <q-tooltip
                        class="bg-black"
                        v-if="buttonStatus('hiz', props.row.id) === 'Mapped'"
                      >
                        <p class="text-lg">
                  ...
                  <q-td key="gain" :props="props">
                    <q-btn
                      round
                      :icon="buttonIcon('gain', props.row.id)"
                      @click="toggleListening('gain', props.row.id)"
                      v-if="props.row.modules.includes('gain')"
                      :class="buttonColor('gain', props.row.id)"
                      class="full-width rounded-borders cursor-pointer"
                    >
                      <q-tooltip
                        class="bg-black"
                        v-if="buttonStatus('gain', props.row.id) === 'Mapped'"
                      >
                        <p class="text-lg">
                  ...
                  <q-td key="volume" :props="props">
                    <q-btn
                      round
                      :icon="buttonIcon('master', props.row.id)"
                      @click="toggleListening('master', props.row.id)"
                      v-if="props.row.modules.includes('volume')"
                      :class="buttonColor('master', props.row.id)"
                      class="full-width rounded-borders cursor-pointer"
                    >
                      <q-tooltip
                        class="bg-black"
                        v-if="buttonStatus('master', props.row.id) === 'Mapped'"
                      >
                        <p class="text-lg">
                  ...
                  <q-td key="solo" :props="props">
                    <q-btn
                      v-if="props.row.modules.includes('solo')"
                      :class="buttonColor('solo', props.row.id)"
                      class="q-pa-md full-width q-px-lg rounded-borders cursor-pointer"
                    >
                      <span></span>
                    </q-btn>
                  </q-td>
                  <q-td key="mute" :props="props">
                    <q-btn
                      v-if="props.row.modules.includes('mute')"
                      :class="buttonColor('mute', props.row.id)"
                      class="q-pa-md full-width q-px-lg rounded-borders cursor-pointer"
                    >
                      <span></span>
                    </q-btn>
                  ...
                  <q-td key="phantom" :props="props">
                    <q-btn
                      v-if="props.row.modules.includes('phantom')"
                      :class="buttonColor('phantom', props.row.id)"
                      class="q-pa-md full-width q-px-lg rounded-borders cursor-pointer"
                    >
                      <span></span>
                    </q-btn>
                  ...
                  <q-td key="polarity" :props="props">
                    <q-btn
                      v-if="props.row.modules.includes('polarity')"
                      :class="buttonColor('polarity', props.row.id)"
                      class="q-pa-md full-width q-px-lg rounded-borders cursor-pointer"
                    >
                      <span></span>
                    </q-btn>
                  ...
                  <q-td key="reverbFX" :props="props">
                    <q-btn
                      v-if="props.row.modules.includes('reverbFX')"
                      :class="buttonColor('reverbFX', props.row.id)"
                      class="q-pa-md full-width q-px-lg rounded-borders cursor-pointer"
                    >
                      <span></span>
                    </q-btn>
                  ...
                  <q-td key="delayFX" :props="props">
                    <q-btn
                      v-if="props.row.modules.includes('delayFX')"
                      :class="buttonColor('delayFX', props.row.id)"
                      class="q-pa-md full-width q-px-lg rounded-borders cursor-pointer"
                    >
                      <span></span>
                    </q-btn>
                  ...
                  <q-td key="chorusFX" :props="props">
                    <q-btn
                      v-if="props.row.modules.includes('chorusFX')"
                      :class="buttonColor('chorusFX', props.row.id)"
                      class="q-pa-md full-width q-px-lg rounded-borders cursor-pointer"
                    >
                      <span></span>
                    </q-btn>
                  ...
                  <q-td key="roomFX" :props="props">
                    <q-btn
                      v-if="props.row.modules.includes('roomFX')"
                      :class="buttonColor('roomFX', props.row.id)"
                      class="q-pa-md full-width q-px-lg rounded-borders cursor-pointer"
                    >
                      <span></span>
                    </q-btn>
                  ...
                  <q-td key="pan" :props="props">
                    <q-btn
                      v-if="props.row.modules.includes('pan')"
                      :class="buttonColor('pan', props.row.id)"
                      class="q-pa-md full-width q-px-lg rounded-borders cursor-pointer"
                    >
                      <span></span>
                    </q-btn>
                  ...
                  <q-td key="select" :props="props">
                    <q-btn
                      v-if="props.row.modules.includes('select')"
                      :class="buttonColor('select', props.row.id)"
                      class="q-pa-md full-width q-px-lg rounded-borders cursor-pointer"
                    >
                      <span></span>
                    </q-btn>
...
const rows = ref<object[]>(
  mixerMidiMappable.filter((x: any) =>
    x.devicesAllowed.includes(mixerStore.mixerInfo.model.toLowerCase()),
  ),
);
...
// }
watch(
  () => mixerStore.mixerInfo,
  (val: any) => {
    rows.value = mixerMidiMappable.filter((x: any) =>
      x.devicesAllowed.includes(val.model.toLowerCase()),
    );
  },
  { deep: true },
);
...
const toggleListening = (type: string, id: string) => {
  const target = type + id;
  console.log(target);
  if (midiStore.currentlyListeningTo === target) {
